...
export default function EditLesson({ getTasks, tasks }) {
    ...
    const getTaskData = async (value) => {
        ...
        const res = await axios.get((process.env.REACT_APP_BASE_URL || 'http://localhost:3001') + '/api/lesson/' + value)
    ...
    const updateTask = async () => {
        ...
        try {
            const res = await axios.put((process.env.REACT_APP_BASE_URL || 'http://localhost:3001') + '/api/lesson', dataJson, {
